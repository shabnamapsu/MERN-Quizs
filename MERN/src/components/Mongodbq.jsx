...
import { mongodbQuiz } from '../assets/data.js'
...
function Mongodbq() {
  ...
  const question = mongodbQuiz[current]
  ...
  const nextQuestion = () => {
    if (current < mongodbQuiz.length - 1) {
      ...
      <div className='bg-blue-800 min-h-screen flex justify-center items-center p-5'>
        <div className='bg-blue-200 w-[60%] p-10 rounded-lg shadow-lg text-center'>
          ...
          <p className='text-xl font-medium'>
            Your Score: <span className="text-blue-950 font-bold">{score}</span> / {mongodbQuiz.length}
          </p>
          <button
